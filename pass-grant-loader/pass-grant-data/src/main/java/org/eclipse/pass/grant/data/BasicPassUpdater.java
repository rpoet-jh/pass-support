...
public class BasicPassUpdater implements PassUpdater {
    ...
    private void updateGrants(Collection<Map<String, String>> results) {
        ...
        for (Map<String, String> rowMap : results) {
            ...
            if (!modeChecked) {
                if (!rowMap.containsKey(CoeusFieldNames.C_GRANT_LOCAL_KEY)) { //we always have this for grants
            ...
            grantLocalKey = rowMap.get(CoeusFieldNames.C_GRANT_LOCAL_KEY);

            String directFunderLocalKey = rowMap.get(CoeusFieldNames.C_DIRECT_FUNDER_LOCAL_KEY);
            String primaryFunderLocalKey = rowMap.get(CoeusFieldNames.C_PRIMARY_FUNDER_LOCAL_KEY);
            ...
            if (!grantMap.containsKey(grantLocalKey)) {
                grant = new Grant();
                grant.setAwardNumber(rowMap.get(CoeusFieldNames.C_GRANT_AWARD_NUMBER));

                String status = rowMap.getOrDefault(CoeusFieldNames.C_GRANT_AWARD_STATUS, null);
            ...
            String abbreviatedRole = rowMap.get(CoeusFieldNames.C_ABBREVIATED_ROLE);
            ...
            if (rowMap.containsKey(CoeusFieldNames.C_UPDATE_TIMESTAMP)) {
                String grantUpdateString = rowMap.get(CoeusFieldNames.C_UPDATE_TIMESTAMP);
    ...
    private void updateUsers(Collection<Map<String, String>> results) {
        ...
        for (Map<String, String> rowMap : results) {
            ...
            if (!modeChecked) {
                if (!rowMap.containsKey(CoeusFieldNames.C_USER_EMPLOYEE_ID)) { //we always have this for users
            ...
            if (rowMap.containsKey(CoeusFieldNames.C_UPDATE_TIMESTAMP)) {
                String userUpdateString = rowMap.get(CoeusFieldNames.C_UPDATE_TIMESTAMP);
    ...
    Funder buildPrimaryFunder(Map<String, String> rowMap) {
        ...
        funder.setName(rowMap.getOrDefault(CoeusFieldNames.C_PRIMARY_FUNDER_NAME, null));
        funder.setLocalKey(rowMap.get(CoeusFieldNames.C_PRIMARY_FUNDER_LOCAL_KEY));
        String policy = rowMap.get(CoeusFieldNames.C_PRIMARY_FUNDER_POLICY);
    ...
    private Funder buildDirectFunder(Map<String, String> rowMap) {
        Funder funder = new Funder();
        if (rowMap.containsKey(CoeusFieldNames.C_DIRECT_FUNDER_NAME)) {
            funder.setName(rowMap.get(CoeusFieldNames.C_DIRECT_FUNDER_NAME));
        }
        funder.setLocalKey(rowMap.get(CoeusFieldNames.C_DIRECT_FUNDER_LOCAL_KEY));
        String policy = rowMap.get(CoeusFieldNames.C_DIRECT_FUNDER_POLICY);
        if (policy != null) {
            String fedoraBaseUrl = System.getProperty("pass.fedora.baseurl");
            fedoraBaseUrl = fedoraBaseUrl.endsWith("/") ? fedoraBaseUrl : fedoraBaseUrl + "/";
            funder.setPolicy(URI.create(fedoraBaseUrl + policy));
