 ...
 *         {@link PackageOptions.Compression.OPTS} and {@link PackageOptions.Archive.OPTS}.  {@code Compression.OPTS} may be specified as
 *         {@link PackageOptions.Compression.OPTS#NONE NONE}, but {@link PackageOptions.Archive.OPTS#NONE Archive.OPTS.NONE} is <em>not</em> a valid
...
public abstract class ThreadedAssemblyIT {
    ...
    public void testMultiplePackageStreams() throws Exception {
        ...
        assertTrue("PackageOptions map must contain Archive.KEY", packageOptions.containsKey(PackageOptions.Archive.KEY));
        assertTrue("PackageOptions map must contain Compression.KEY",
                   packageOptions.containsKey(PackageOptions.Compression.KEY));
        assertFalse("PackageOptions map must contain a valid Archive option " +
                    "(Archive.OPTS.NONE is not supported)", packageOptions.containsValue(PackageOptions.Archive.OPTS.NONE));
        ...
        results.forEach((submission, future) -> {
            try {
                ...
                Assert.assertEquals(stream.metadata().archive(), packageOptions.get(PackageOptions.Archive.KEY));
                Assert.assertEquals(stream.metadata().compression(), packageOptions.get(PackageOptions.Compression.KEY));
        ...
        packages.forEach((submission, packageFile) -> {
            ...
            File dir;
            try {
                dir = openArchive(packageFile, (PackageOptions.Archive.OPTS) packageOptions.get(PackageOptions.Archive.KEY),
                                  (PackageOptions.Compression.OPTS) packageOptions.get(PackageOptions.Compression.KEY));
     ...
     * {@link Assembler#assemble(DepositSubmission, Map)}.  {@link PackageOptions.Compression.OPTS} and {@link PackageOptions.Archive.OPTS} must be
     * supplied.  Note that {@link PackageOptions.Archive.OPTS#NONE} <strong>is not supported</strong> at this time.
     *
